...
//1.클라이언트로 부터 'id'값을 요청 본문에서 가져옴
    const id = req.body.id;
//2. 주어진 id에 해당하는 게시글 정보를 조회함
const sqlQuery = 
"SELECT BOARD_ID, BOARD_TITLE, BOARD_CONTENT FROM BOARD WHERE BOARD_ID = ?;"
//3 .db.query를 사용하여 myspl 데이터베이스에 query를 실행함
db.query(sqlQuery,(err, result) => {
//4. query실행 후에, error발생 -> 적절히 처리 또는 결과를 클라이언트에게 응답으로 보냄
res.send(result)
})
});
...
app.post("/insert",(req, res) => {
    const title = req.body.title; //새로운 글을 쓰는데 숫자가 자동으로 1씩 증가하면서 타이틀과 내용을 mysql에서 레코드한다
    const content = req.body.title;
    
    const sqlQuery=
    "INSERT INTO BOARD (BOARD_TITLE, BOARD_CONTENT, REGISTER_ID) VALUES (?, ?, '진히')"
